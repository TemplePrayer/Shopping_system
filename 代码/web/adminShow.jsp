...
<div style="margin-top: 30px;">
    ...
    <button onclick="shopAdd()" style="height: 30px;width: 90px;margin-left: 50px;">添加商品</button>
    <button onclick="logout()" style="height: 30px;width: 90px;color: red;">退出系统</button>
</div>

<div style="margin-top: 30px;">
    <form method="post" action="<%=PATH%>/toAdmin" >
        <label style="font-size: 17px">请输入商品名称：</label>
    ...
    <table style="margin: auto">
        ...
        <c:forEach var="item" items="${requestScope.result}">
            <tr class="double">
                ...
                <td>${item.num}</td>
                <td>
                    <button onclick="shopUpdate(${item.id})">修改</button>
                    <button onclick="shopDelete(${item.id})" style="color: red;">删除</button>
                </td>
            </tr>
        </c:forEach>
    </table>
...
<script>
    ...
    //添加商品
    function shopAdd(){
        window.location.href = "<%=PATH%>/shopAdd";
    }
    //删除商品
    function shopDelete(id){
        var b = window.confirm("你确定要删除这条记录吗?");
        if(b == true){
            $.ajax({
                url: "<%=PATH%>/shopDelete",
                type: "post",
    ...
    //修改商品信息
    function shopUpdate(id){
        window.location.href = "<%=PATH%>/updateShop?id=" + id;
    }
